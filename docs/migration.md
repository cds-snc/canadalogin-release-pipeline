...
4. Run CI and create a protected `v1.0.6` release tag. Never move a published release tag.
...
The shared workflow serializes pipeline runs per caller repository and pipeline ID. The optional `pipeline_id` input defaults to `default`, so existing callers need no change. For multiple independent pipelines in one repository, give each caller workflow a distinct `pipeline_id` and include the same value in its caller-level `concurrency` group; keep that block because it protects the caller's other release jobs and makes the intended queue behavior visible.

For example, two test entrypoints can use `test1` and `test2`:

```yaml
jobs:
	release:
		concurrency:
			group: canadalogin-release-test1-${{ github.event_name == 'pull_request' && format('pr-{0}', github.event.pull_request.number) || 'deploy' }}
			queue: max
...
Use a separate configuration path, GitHub environment namespace, artifact bucket or prefix, ECR repository, and deployment resources for each test pipeline. Keep release-please disabled for concurrent test-only entrypoints unless they intentionally share one repository release stream.
