...
4. Run CI and create a protected `v1.0.0` release tag. Never move a published release tag.
...
3. Pin the caller to a reviewed, protected release tag or full commit SHA. A full SHA is GitHub's immutable form; do not use `main` in a production caller.
...
6. Ensure the caller grants `contents: write`, `issues: write`, `pull-requests: write`, and `id-token: write` to the reusable workflow job.
